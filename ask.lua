...
local system_prompt = [[
You are an AI programming assistant integrated into a code editor. Your purpose is to help the user with programming tasks as they write code. Just complete what comes next. Your response will be placed directly at the end of the file. When writing code, return just the code, with no additional information or explainations.
]]
...
function API.decode(chunk)
    if string.sub(chunk, 1, 6) == "data: " and chunk ~= 'data: [DONE]' then
        local line = vim.json.decode(string.sub(chunk, 7))
...
local function get_or_create_response_buffer()
    if response_buffer and vim.api.nvim_buf_is_valid(response_buffer) then
        ...
        vim.api.nvim_buf_set_lines(response_buffer, 0, -1, false, {})
        -- Switch to the window containing the response buffer
        for _, win in ipairs(vim.api.nvim_list_wins()) do
            if vim.api.nvim_win_get_buf(win) == response_buffer then
        ...
        vim.api.nvim_win_set_buf(0, response_buffer)
        return response_buffer
    else
        ...
        vim.api.nvim_command("botright vsplit")
        vim.api.nvim_win_set_buf(0, response_buffer)
        vim.api.nvim_buf_set_option(response_buffer, "buftype", "nofile")
        return response_buffer
    end
end
...
vim.api.nvim_create_user_command("Modify", function(opts)
...
vim.api.nvim_create_user_command("Model", function(opts)
...
function M.setup()
    vim.api.nvim_set_keymap('n', '.', [[<cmd>lua require('ask').ask_command_wrapper()<CR>]], { noremap = true, silent = true })
end
